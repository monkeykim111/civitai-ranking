...
최신 버전의 thumbs up/down 기준으로 OP 조건을 통과한 모델만 추려
type별·base model별 개수와 총합을 출력한다.
...
OP 판정 기준 (Checkpoint / LoRA 분리):

```
total = thumbs_up + thumbs_down
ratio = thumbs_up / total
OP 조건: ratio >= MIN_RATIO AND total >= MIN_REVIEWS   (total == 0 이면 제외)

Checkpoint: ratio >= 0.95, reviews >= 500
LoRA:       ratio >= 0.97, reviews >= 1000   (LoRA 후보 수가 많아 더 엄격)
```

페이지 상한은 type별로 분리한다. `sort=Highest Rated` 정렬상 OP 후보는 앞쪽에
몰리므로, OP 0건 페이지가 연속되면 조기 종료한다.

```python
CHECKPOINT_MAX_PAGES_PER_BASE_MODEL = 30
LORA_MAX_PAGES_PER_BASE_MODEL = 15
STOP_AFTER_EMPTY_OP_PAGES = 2
```
...
- `state.json`이 없으면 **baseline 모드**: `is_new=false`, delta/pct 생략, 절댓값 랭킹만
...
렌더링 섹션: 🏆 Checkpoint 누적 TOP · 🎨 LoRA 누적 TOP · 📈 Checkpoint 다운로드 급상승
· 🚀 LoRA 다운로드 급상승 · ❤️ Checkpoint 좋아요 급상승 · 💜 LoRA 좋아요 급상승.
모델명은 `<url|name>` 클릭 링크로 표시되고, baseline이 아니면 🆕 / ▲증가분 / (+%)을
함께 보여준다. baseline이면 이 표시는 생략된다.
