"""Civitai Weekly Digest — Phase 2 (수집 + OP 필터 + 상태/델타/랭킹).

이 스크립트는 Civitai 공식 REST API에서 지정된 base model 계열의 모델을 수집하고,
최신 버전의 thumbs up/down 통계를 기준으로 OP(Overwhelmingly Positive)에 가까운
고평가 모델만 필터링한 뒤, 지난 실행 state와 비교해 신규 여부·증가분을 계산하고
Checkpoint / LoRA를 분리한 랭킹 섹션(digest)을 만든다.

수집 대상:
- Checkpoint 후보
- 동일 base model 계열의 호환 LoRA 후보 (same_base_model_lora 기준)

> 참고: Civitai API만으로는 특정 LoRA가 특정 Checkpoint에서 정확히 "파생"되었는지
> 항상 보장하기 어렵다. 따라서 초기 운영에서는 동일 base model을 공유하는 LoRA를
> "호환 LoRA 후보"로 수집한다. 이름/태그/설명 기반의 related_lora 매칭은 추후 추가한다.

산출물:
- state.json            : 비교용 작업 기억 (seen)
- history/YYYY-MM-DD.json: 주간 전체 스냅샷
- digest.json           : 랭킹 섹션 (Slack/Claude 입력)
- checkpoint_candidates_debug.json / lora_candidates_debug.json : 사람 확인용 덤프

Phase 3 이후(Slack 발송, Claude 추천 코멘트, Routine 연결)는 구현하지 않는다.
"""
...
# Turbo / Lightning 계열은 수집 대상에서 제외한다.
TARGET_BASE_MODELS = [
    "SDXL 1.0",
    "Pony",
    "Illustrious",
...
# OP 판정 임계값 — Checkpoint / LoRA 분리
CHECKPOINT_OP_MIN_RATIO = 0.95
CHECKPOINT_OP_MIN_REVIEWS = 500

LORA_OP_MIN_RATIO = 0.97
LORA_OP_MIN_REVIEWS = 1000
...
STOP_AFTER_EMPTY_OP_PAGES = 2                # OP 0건 페이지가 연속 N번이면 조기 종료

# 재시도/백오프 제어
MAX_RETRIES = 3          # 요청 실패 시 최대 재시도 횟수
BACKOFF_BASE_SEC = 2.0   # 백오프 기준 시간 (지수 백오프)
REQUEST_TIMEOUT_SEC = 60 # 단일 요청 타임아웃

# 랭킹 섹션 개수
N_CHECKPOINT_CUMULATIVE = 10
N_LORA_CUMULATIVE = 10
...
# 모듈 전역: fetch_models가 (model_type, base_model)별 진단 통계를 기록한다.
# 구조: FETCH_STATS[model_type][base_model] =
#       {fetched_count, page_count, truncated, stop_reason}
FETCH_STATS: dict[str, dict[str, dict]] = {}
...
def _request_json(url: str, token: str | None, params: dict | None = None) -> dict:
    ...
    """
    headers = {
        "User-Agent": "civitai-weekly-digest/phase2",
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # 429/5xx 또는 네트워크 오류 시 백오프 후 재시도
            last_error = exc
    ...
    # 모든 재시도 실패 시 예외를 다시 던진다.
    assert last_error is not None
    raise last_error


def _first_page_params(base_model: str, model_type: str) -> dict:
    """base model + type에 대한 첫 페이지 요청 쿼리 파라미터를 만든다."""
    return {
        "types": model_type,
...
def is_op(up: int, down: int, min_ratio: float, min_reviews: int) -> bool:
    """OP(Overwhelmingly Positive) 판정.

    total == 0 이면 False.
    up / total >= min_ratio 이고 total >= min_reviews 이면 True.
    """
    total = up + down
    if total == 0:
        return False
    ratio = up / total
    return ratio >= min_ratio and total >= min_reviews


def _count_op_in_page(page_items: list[dict], min_ratio: float, min_reviews: int) -> int:
    """페이지 내 OP 통과 모델 수를 센다 (조기 종료 판단용)."""
    count = 0
    for model in page_items:
        up, down, _ = latest_stats(model)
        if is_op(up, down, min_ratio, min_reviews):
            count += 1
    return count
...
def fetch_models(
    ...
    """단일 base model + type에 대해 Civitai API를 호출하고 모델 목록을 반환한다.

    - API query의 `types` 값으로 model_type을 사용한다.
    - cursor/nextPage pagination을 처리한다.
    - base model당 최대 max_pages 페이지까지만 수집한다.
    - OP 0건 페이지가 STOP_AFTER_EMPTY_OP_PAGES번 연속되면 조기 종료한다.
      (sort=Highest Rated 정렬이므로 OP 후보는 앞쪽에 몰려 있다.)
    - page_count / truncated / stop_reason을 모듈 전역 FETCH_STATS에 기록한다.

    stop_reason 값:
      - "max_pages"      : 페이지 상한에 걸려 멈춤 (truncated=True)
      - "empty_op_pages" : OP 0건 페이지가 연속되어 조기 종료
      - "no_next"        : 응답에 nextCursor / nextPage가 없어 정상 종료
      - "empty_page"     : 페이지에 항목이 없어 종료
    """
    items: list[dict] = []
    page_count = 0
    truncated = False
    ...
    # 첫 페이지는 base URL + 쿼리 파라미터, 이후 페이지는 cursor 또는 nextPage URL을 따라간다.
    url: str | None = API_BASE_URL
    params: dict | None = _first_page_params(base_model, model_type)

    while True:
        if page_count >= max_pages:
            # 더 가져올 수 있었지만 페이지 제한 때문에 멈춘 경우
            truncated = True
        ...
        # OP-aware 조기 종료: OP 0건 페이지가 연속되면 더 깊이 가지 않는다.
        if _count_op_in_page(page_items, op_min_ratio, op_min_reviews) == 0:
            empty_op_streak += 1
        else:
            empty_op_streak = 0
        if empty_op_streak >= STOP_AFTER_EMPTY_OP_PAGES:
            stop_reason = "empty_op_pages"
            break

        # 다음 페이지 결정: nextCursor 우선, 없으면 nextPage
        ...
        if next_cursor:
            url = API_BASE_URL
            params = _first_page_params(base_model, model_type)
    ...
    FETCH_STATS.setdefault(model_type, {})[base_model] = {
...
def _build_candidate(model: dict, version: dict, base_model: str, model_type: str) -> dict:
    ...
    # description은 HTML이 섞여 있을 수 있어 길이만 잘라 보관한다.
...
def collect_raw_candidates(
    ...
    """특정 type의 전체 후보(OP 필터 적용 전)를 수집한다.

    - 전체 base model을 순회한다.
    - version_id(type+version_id) 기준으로 중복을 제거한다.
    """
    raw: list[dict] = []
    seen_version_ids: set[int] = set()

    for base_model in TARGET_BASE_MODELS:
        print(f"[fetch] {model_type} · {base_model} 수집 중...")
        models = fetch_models(
            base_model, model_type, token, max_pages, op_min_ratio, op_min_reviews
        )
        for model in models:
            version = latest_version(model)
            if version is None:
...
def filter_op(raw: list[dict], min_ratio: float, min_reviews: int) -> list[dict]:
    """raw 후보 목록에서 OP 기준을 통과한 후보만 반환한다."""
    return [
        c for c in raw
        if is_op(c["thumbs_up"], c["thumbs_down"], min_ratio, min_reviews)
    ]


def collect_checkpoint_candidates(token: str | None) -> list[dict]:
    """Checkpoint OP 후보를 수집한다 (Checkpoint 기준 적용)."""
    raw = collect_raw_candidates(
        token,
        CHECKPOINT_TYPES[0],
        CHECKPOINT_MAX_PAGES_PER_BASE_MODEL,
        CHECKPOINT_OP_MIN_RATIO,
        CHECKPOINT_OP_MIN_REVIEWS,
    )
    return filter_op(raw, CHECKPOINT_OP_MIN_RATIO, CHECKPOINT_OP_MIN_REVIEWS)


def collect_lora_candidates(token: str | None) -> list[dict]:
    """동일 base model 계열의 호환 LoRA OP 후보를 수집한다 (LoRA 기준 적용)."""
    raw = collect_raw_candidates(
        token,
        LORA_TYPES[0],
        LORA_MAX_PAGES_PER_BASE_MODEL,
        LORA_OP_MIN_RATIO,
        LORA_OP_MIN_REVIEWS,
    )
    return filter_op(raw, LORA_OP_MIN_RATIO, LORA_OP_MIN_REVIEWS)
...
def load_state(path: str) -> tuple[dict, bool]:
    """state.json을 읽어 (state, is_baseline)을 반환한다.

    파일이 없으면 baseline 모드(빈 state, is_baseline=True).
    """
...
def compute_deltas(candidate: dict, state: dict, is_baseline: bool) -> dict:
    ...
    - baseline: is_new=False, 모든 delta/pct = None
    - 신규(state.seen에 없음): is_new=True, delta/pct = None
    - 기존: delta = max(0, current - previous), pct = delta/previous*100 (previous=0이면 None)
    """
    if is_baseline:
        return {
            "is_new": False,
            "thumbs_up_delta": None,
            "thumbs_up_pct": None,
    ...
    if prev is None:
        # 신규 모델은 비교 대상이 없으므로 delta를 계산하지 않는다.
        return {
            "is_new": True,
            "thumbs_up_delta": None,
            "thumbs_up_pct": None,
...
def update_state(state: dict, candidates: list[dict], today: str) -> dict:
    ...
    - 기존 항목은 first_seen을 유지하고 last_* 값을 갱신한다.
    - 신규 항목은 first_seen=today로 추가한다.
    """
    seen = state.get("seen", {})
    for c in candidates:
        key = _seen_key(c)
...
def _top_cumulative(candidates: list[dict], n: int) -> list[dict]:
    """thumbs_up 절댓값 내림차순 상위 n개."""
    return sorted(candidates, key=lambda c: c["thumbs_up"], reverse=True)[:n]


def _top_surge(
    candidates: list[dict], n: int, delta_key: str, pct_key: str, guard: int
) -> list[dict]:
    """증가율(pct_key) 내림차순 상위 n개. 단 delta_key >= guard 이고 pct가 있는 경우만."""
    eligible = [
        c for c in candidates
        if c.get(delta_key) is not None
        and c[delta_key] >= guard
        and c.get(pct_key) is not None
    ]
...
def build_sections(checkpoint: list[dict], lora: list[dict], is_baseline: bool) -> dict:
    ...
    baseline 모드에서는 surge 섹션을 빈 배열로 둔다.
    """
    sections = {
        "checkpoint_cumulative": _top_cumulative(checkpoint, N_CHECKPOINT_CUMULATIVE),
        "lora_cumulative": _top_cumulative(lora, N_LORA_CUMULATIVE),
...
def _print_run_summary(today: str, is_baseline: bool, counts: dict, sections: dict) -> None:
    ...
    print(f"checkpoint_op_count: {counts['checkpoint_op_count']}")
    print(f"lora_op_count: {counts['lora_op_count']}")
...
def main() -> None:
    ...
    if token:
        print("[auth] CIVITAI_TOKEN 사용 (인증 호출)")
    else:
        print("[auth] CIVITAI_TOKEN 없음 (public 호출)")

    today = datetime.date.today().isoformat()
    state, is_baseline = load_state(STATE_PATH)

    # raw 수집 → OP 필터
    raw_checkpoint = collect_raw_candidates(
        token, CHECKPOINT_TYPES[0], CHECKPOINT_MAX_PAGES_PER_BASE_MODEL,
        CHECKPOINT_OP_MIN_RATIO, CHECKPOINT_OP_MIN_REVIEWS,
    )
    raw_lora = collect_raw_candidates(
        token, LORA_TYPES[0], LORA_MAX_PAGES_PER_BASE_MODEL,
        LORA_OP_MIN_RATIO, LORA_OP_MIN_REVIEWS,
    )
    checkpoint = filter_op(raw_checkpoint, CHECKPOINT_OP_MIN_RATIO, CHECKPOINT_OP_MIN_REVIEWS)
    lora = filter_op(raw_lora, LORA_OP_MIN_RATIO, LORA_OP_MIN_REVIEWS)
    ...
    counts = {
        "checkpoint_op_count": len(checkpoint),
        "lora_op_count": len(lora),
    }
